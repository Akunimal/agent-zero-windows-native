...
- Chat saves write and fsync a same-directory temporary file, atomically replace `chat.json`, and fsync the directory on POSIX. Windows skips the unsupported directory fsync while retaining file fsync and same-volume atomic replacement, so an interrupted save cannot truncate the previous chat.
